...
class Biped:
	...
	up_limits = np.array([30.0, 120.0, 30.0])
	...
	def forward_kinematics(self, leg_down = 2):
		...
		if (leg_down == 0):
			...
			print 'calculating for both feet down'
	...
	def ZMP_double_cond(self, rfoot_back = True):
	...
	def finite_differencing(self):
		if (self.status == 0):
			...
			l_knee_plus = max(np.pi/3.0, self.l_angles[2] + np.pi/3.0 * self.dt)
			...
			if (self.prev_status == 0):
				#still move the left leg first
				l_knee_orig = self.l_angles[2]
			elif (self.prev_status == 1):
				#still move the right leg first
				r_knee_orig = self.l_angles[2]
	...
	def trajectory(self):
		...
		elif (self.status == 2 and self.prev_status == 0):
			self.r_angles[1] = max(0.0, self.r_angles[1] - np.pi/3.0 * self.dt) #increment hip swing
		elif (self.status == 2 and self.prev_status == 1):
			self.l_angles[1] = max(0.0, self.l_angles[1] - np.pi/3.0 * self.dt) #increment hip swing
		...
		if (self.status == 0 and self.lfoot_pos[1] < 1e-6):
			...
			if (self.prev_status == 0 and self.rfoot_pos[1] > 1e-2):
				self.status = 1
				self.prev_status = 2
				print 'double support phase state change to left leg movement'
			if (self.prev_status == 1 and self.lfoot_pos[1] > 1e-2):
				self.status = 0
				self.prev_status = 2
				print 'double support phase state change to left leg movement'
